...
class open_goverment extends CI_Controller
{
	...
	public function view($showType, $aid)
	{
		...
		if ($showType == 'article') {
			...
			if ($showType == 'zsyw') {
				$url = 'http://www.zs.gov.cn/main/zwgk/newsview/index.action?id=' . $aid;
				$data['category_id'] = 'zsyw';
				$data['category_name'] = '中山要闻';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'sjtz') {
				$url = 'http://www.zs.gov.cn/main/zwgk/newsview/index.action?id=' . $aid;
				$data['category_id'] = 'sjtz';
				$data['category_name'] = '市级通知';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'rsxx') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'rsxx';
				$data['category_name'] = '人事信息';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'zcwj') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'zcwj';
				$data['category_name'] = '政策文件';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'fzjh') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'fzjh';
				$data['category_name'] = '规划计划';
				$content = $this->getPageContent($url); 
			} elseif ($showType == 'zfwork') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'zfwork';
				$data['category_name'] = '政府工作报告';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'tjsj') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'tjsj';
				$data['category_name'] = '统计数据';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'zsyz') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'zsyz';
				$data['category_name'] = '招商引资';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'czxx') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'czxx';
				$data['category_name'] = '财政信息';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'gkzn') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'gkzn';
				$data['category_name'] = '信息公开指南';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'gkgd') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/rightview/index.action?id=7730';
				$data['category_id'] = 'gkgd';
				$data['category_name'] = '信息公开规定';
				$content = $this->getPage_gkgd($url);
			} elseif ($showType == 'gkbg') {
				$url = 'http://www.zs.gov.cn/main/zwgk/open/view/index.action?id=' . $aid;
				$data['category_id'] = 'gkbg';
				$data['category_name'] = '公开年度报告';
				$content = $this->getPageContent($url);
			} elseif ($showType == 'sqgk') {
				header("location:http://www.zs.gov.cn/main/zwgk/open/request3/index.action?pubcode=1-04C-20");
				exit();
			} else {
				action_message("/index.php", 3, '您访问的页面不存在！', 0);
				exit();
			...
			$str1= preg_match("/downloaddb\.sl/i",$content['content']); 
			$str2= preg_match("/www.banfu.gov.cn\//i",$content['content']);   
			if(!$str1&&$str2){
			    $str =preg_match("/href=\"(.*)\" /", $content['content'], $href); //获取a标签里的链接
			        header("location: $href[1]");			 
			}
	...
	public function about(){
		...
		$data = array('navs' => $navs,'nav_selected' => 0,'en_navs' => $en_navs,'navs_url' => $navs_url);
	   	$this->load->model('model_open_goverment', 'mog');
		$topad = $this->mog->get_guanyu(); //关于协会
	    $data['about']=$topad;
		 
	    $this->load->view('index/header',  $data);
		$this->load->view('index/guanyu');
		$this->load->view('index/footer');
	}
}
