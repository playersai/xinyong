...
class Interaction extends CI_Controller
{
	...
	public function search($backurl = null)
	{
		...
		if (! empty($pdata['handle_id']) and ! empty($pdata['password'])) {
			$_where = "handle_id='" . mysql_real_escape_string($pdata['handle_id']) . "' and query_password='" . mysql_real_escape_string($pdata['password']) . "'";
			$_limit = 20;
			$result = $this->iim->do_search_handle($_where, $_limit);
			if ($result) {
				$this->load->model('Index_interactionm', 'iim');
		        $reply_rows = $this->iim->get_feedback_reply_by_fcid($result[0]->f_c_id);
			    $fc_rows = $this->iim->get_fb_content_by_fcid2($result[0]->f_c_id);
				$data = array('topad' => $topad,'navs' => $navs,'nav_selected' => 4,'en_navs' => $en_navs,'navs_url' => $navs_url,'srows' => $result[0], 'reply_rows'=> $reply_rows[0],'fc_rows'=>$fc_rows[0]);
